...
template<class tchar> DECLARE_INTERFACE_(IffdshowBaseT,IUnknown)
{
 ...
 STDMETHOD (getParamStr)(unsigned int paramID,tchar *buf,unsigned int buflen) PURE;
 ...
 STDMETHOD (getParamName)(unsigned int i,tchar *buf,unsigned int len) PURE;
 ...
 STDMETHOD (savePresetMem)(void *buf,unsigned int len) PURE; //if len=0, then buf should point to int variable which will be filled with required buffer length
 STDMETHOD (loadPresetMem)(const void *buf,unsigned int len) PURE;
 STDMETHOD (getParamName3)(unsigned int i,const tchar* *namePtr) PURE;
 STDMETHOD (getInCodecString)(tchar *buf,unsigned int buflen) PURE;
 STDMETHOD (getOutCodecString)(tchar *buf,unsigned int buflen) PURE;
